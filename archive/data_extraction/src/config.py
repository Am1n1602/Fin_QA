...
def _load_companies() -> list[dict]:
    # data_extraction/data/ was deliberately left behind at the repo root when
    # data_extraction/src/ moved into archive/ -- finqa_v2's own dataset-rebuild
    # pipeline still reads from it at that original path. Climb past the archive/
    # wrapper (parents[3] = repo root from this file) rather than assuming BASE_DIR's
    # "data" subfolder moved along with the code.
    cache_path = Path(__file__).resolve().parents[3] / "data_extraction" / "data" / "universe" / "nifty50_constituents.json"
...
# data_extraction/data/ stays at the repo root (not moved into archive/ with this
# code) since finqa_v2's own dataset-rebuild pipeline reads from it directly.
BASE_DIR = Path(__file__).resolve().parents[3] / "data_extraction"
